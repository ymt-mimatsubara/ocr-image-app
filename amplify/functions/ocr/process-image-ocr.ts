...
**抽出ルール：**
1.注文日時の読み取り（重要）：
  ・「ご注文日」の項目を慎重に確認し、年月日と時刻を正確に読み取る
  ・年（yyyy）が表示されていない場合は、現在年（2025年）を自動的に付加する
  ・例：「1月15日」→「2025-01-15」、「12/25」→「2025-12-25」
  ・年が表示されている場合はその年を使用する
  
2.文字認識の精度向上（最重要）：
  ・数字の判別を慎重に行う：0と6、1と7、2と5、3と8、4と9の区別を特に注意
  ・カタカナの判別を慎重に行う：ロとロ、ニと二、ハとバ、ツとシ、ソとン等
  ・漢字の判別を慎重に行う：似た形の漢字（例：土と士、未と末、己と已）
  ・文字の一部が欠けている場合は、前後の文脈から推測する
  ・不明確な文字は「?」で表記せず、最も可能性の高い文字を選択する
...
3.商品名の読み取り（重要）：
  ...
  ・特に数字とカタカナ・漢字の組み合わせは慎重に判断する

4.金額は¥マークやカンマを除いた数値のみを抽出
5.日付は西暦4桁-月2桁-日2桁の形式で統一（例：2024年8月31日 → 2024-08-31）
6.商品が複数ある場合はorderDetails配列に全て含める
7.itemIdは各商品に対して「注文番号+001」「注文番号+ITEM_002」のような形式で自動生成
（注文番号が「sxfn-243266」の場合の例：「sxfn-243266_001」「sxfn-243266_002」）
8.読み取れない項目は空文字またはnullを設定
9.日本語の商品名はそのまま保持

**商品名読み取りの手順：**
1.商品画像を特定する
2.その商品画像の右側にある商品名テキスト領域を特定する
3.商品名の開始位置から終了位置まで、文字を一つずつ慎重に確認する
...
・似ている文字の判別基準：
  - 数字：線の太さ、角度、接続部分を注意深く観察
  - カタカナ：点や線の位置、長さを正確に判断
  - 漢字：画数、部首、バランスを総合的に判断
・商品名に含まれるスペースや改行も正確に反映してください
・文字が不鮮明な場合は、商品名の文脈や一般的な商品名パターンも考慮する

**年補完の具体例：**
・「1月15日 10:30」→「2025-01-15」
・「12/25」→「2025-12-25」  
・「2024年3月10日」→「2024-03-10」（年が明記されている場合はそのまま使用）
